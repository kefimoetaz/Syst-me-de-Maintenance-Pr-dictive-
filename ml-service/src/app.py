...
from src.lstm_predictor import predict_for_machine
...
def get_high_risk_machines():
    ...
    try:
        ...
        # Get machines whose LATEST prediction is HIGH or CRITICAL
        cursor.execute("""
            SELECT
                machine_id,
                prediction_date,
...
@app.route('/api/lstm/predict/<int:machine_id>', methods=['GET'])
@require_auth
def lstm_predict(machine_id):
    """
    Run LSTM time-series prediction for a machine.
    Returns: { prediction, risk_level, anomaly, explanation }
    """
